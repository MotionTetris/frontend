...
export function collisionParticleEffect(
  ...
  for (let i = 0; i < 30; i++) {
      ...
      const particle = new PIXI.Sprite(collisionTexture); // 파티클 이미지
...
export function generateParticleTexture(app: PIXI.Application): PIXI.Texture {
  const particleGraphics = new PIXI. Graphics();
  particleGraphics.beginFill(0xffb6c1);
  particleGraphics.drawCircle(0, 0, 5);
  particleGraphics.endFill();
  return app.renderer.generateTexture(particleGraphics);
}
...
export function performRotateEffect(rectangle: PIXI.Graphics, app: PIXI.Application, color: number) {
  ...
  export function createLineEffect(i: number, app: PIXI.Application, lines: PIXI.Graphics[]): void {
    ...
    app.stage.addChild(line); // stage에 추가
}